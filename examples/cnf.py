    ...
    for noise_level, num_samples in itertools.product(noise_levels, num_samples_list):
        ...
        try:
            for itr in range(1, args.niters + 1):
                ...
                x, logp_diff_t1 = get_batch(num_samples, noise_level)
        ...
        if args.viz:
            viz_samples = num_samples
            viz_timesteps = 41
            target_sample, _ = get_batch(viz_samples, noise_level)
            ...
            loss_df = pd.DataFrame({'Average Loss': average_loss_list, 'Current Loss': current_loss_list})
            excel_filename = os.path.join(results_dir, f'loss_data_{noise_level}_{num_samples}.xlsx')
